...
public class MainGameUIScript : MonoBehaviour
{
    ...
    private void SetUpControlsTutorials()
    {
        ...
        if (boostTutorial != null)
        {
            ...
            {
                1 => $"Drücke <color={highlight}>{boostKeyString} <color={white}>, um den <color={highlight}>Raketenboost <color={white}>zu nutzen und alle UFO-Prefabs zu zerstören.\n<size=30><alpha=#80>Drücke [Alt], um das Tutorial zu überspringen.",
                2 => $"Premi <color={highlight}>{boostKeyString} <color={white}>per attivare il <color={highlight}>boost <color={white}>e distruggere tutti gli prefab UFO.\n<size=30><alpha=#80>Premi [Alt] per saltare il tutorial.",
                3 => $"Appuyez sur <color={highlight}>{boostKeyString} <color={white}>pour vous <color={highlight}>propulser <color={white}>et détruire tous les prefabs UFO.\n<size=30><alpha=#80>Appuie sur [Alt] pour pour passer le tutoriel.",
                _ => $"Press <color={highlight}>{boostKeyString} <color={white}>to <color={highlight}>boost up <color={white}>and destroy all UFO prfabs.\n<size=30><alpha=#80>Press [Alt] to skip the tutorial."
            };
        }
    }
